...
def _stacked_bar_plot(data, **kwargs):
    ...
    kwargs.pop('color', None)  # Remove color passed by FacetGrid to use colormap instead
...
def plot_complete_profile(
    ...
    for i, total in enumerate(totals):
        ax.text(i, total + (total*0.01), f"{int(total)}", ha='center', va='bottom', fontweight='bold')
...
def perform_pca_grid(
    ...
    subsets = {
        ...
        "High-Variance Stages": list(df[stage_cols].var().sort_values(ascending=False).head(min(5, len(stage_cols))).index)
    }
    ...
    for i, (name, cols) in enumerate(subsets.items()):
        ...
        scatter = sns.scatterplot(
            x=X_pca[:, 0], y=X_pca[:, 1], hue=df[label_col], 
            ax=axes[i], palette="Set2", alpha=0.8, s=80, edgecolor='w'
        )
    ...
    plt.suptitle(f"PCA Unsupervised Analysis Grid: {title}", fontsize=20, fontweight='bold', y=1.05)
    plt.tight_layout()
    plt.savefig(output_path)
    plt.close()
...
def perform_tsne_grid(
    ...
    for i, perp in enumerate(perplexities):
        ...
        sns.scatterplot(
            x=X_tsne[:, 0], y=X_tsne[:, 1], hue=df[label_col], 
            ax=axes[i], palette="Set1", alpha=0.8, s=80, edgecolor='w'
        )
    ...
    plt.suptitle(f"t-SNE Parameter Grid Search: {title}", fontsize=20, fontweight='bold', y=1.05)
    plt.tight_layout()
    plt.savefig(output_path)
    plt.close()
...
def generate_report(
    ...
    if not df_flor.empty:
        ...
        plot_faceted_stacked_bars(
            df_flor, flor_stages, group_col="tratamento", facet_col=facet_col,
            row_facet=row_facet,
        ...
        plot_complete_profile(
            df_flor, flor_stages, title="Flor Totals (Complete Profile)",
            output_path=os.path.join(output_dir, "flor_complete_profile.png")
        )
        
        # Unsupervised Grids
        perform_pca_grid(
            df_flor, flor_stages, title="Flor PCA",
            output_path=os.path.join(output_dir, "flor_pca_grid.png")
        )
        perform_tsne_grid(
            df_flor, flor_stages, title="Flor t-SNE",
    ...
    if not df_fruto.empty:
        # Stacked evolution by Treatment and Facets
        plot_faceted_stacked_bars(
            df_fruto, fruto_stages, group_col="tratamento", facet_col=facet_col,
            row_facet=row_facet,
        ...
        # Complete Aggregated Profile
        plot_complete_profile(
            df_fruto, fruto_stages, title="Fruto Totals (Complete Profile)",
            output_path=os.path.join(output_dir, "fruto_complete_profile.png")
        )
        
        # Unsupervised Grids
        perform_pca_grid(
            df_fruto, fruto_stages, title="Fruto PCA",
            output_path=os.path.join(output_dir, "fruto_pca_grid.png")
        )
        perform_tsne_grid(
            df_fruto, fruto_stages, title="Fruto t-SNE",
